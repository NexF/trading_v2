...
class EastMoneySession(BaseSession):
    ...
    async def __login__(self):
        ...
        browser = await launch(
            options={"args": ["--no-sandbox", "--window-size=1920,1080"]}
        )
        ...
        await page.type(
            "#txtZjzh", self.user
        )  # 设置账户，selector可以通过chrome开发者模式右键获取
        await page.type("#txtPwd", self.passwd)  # 设置密码
        VaildImg = await page.xpath(
            '//*[@id="imgValidCode"]'
        )  # 得到验证码对象，通过xpath
        ...
        if page.url != self.url:
            _logger.warning("登录成功")
            ret = await page.cookies()
        else:
            _logger.warning("登录失败，可能是验证码识别错误")
            ret = -1
